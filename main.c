...
#define LED      NRF_GPIO_PIN_MAP(0,16)                                             /*********** led pin define **********/
#define LED1     NRF_GPIO_PIN_MAP(0,15)
...
int main(void)
{
    ...
    //NRF_LOG_INFO("PDM Init status: %d ", result);
...
void Test()
{
    static uint32_t timeTickCount;
    static uint8_t ledValue;  
    timeTickCount++;
    if((timeTickCount % (uint32_t)500000) == (uint32_t)0) // ~ 1s
    {
